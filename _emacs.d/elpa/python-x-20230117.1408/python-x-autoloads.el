;;; python-x-autoloads.el --- automatically extracted autoloads  -*- lexical-binding: t -*-
;;
;;; Code:
...
(register-definition-prefixes "python-help-mode" '("python-help-"))
...
result of the expression on the shell." t nil)
...
Evaluate the current paragraph." t nil)
...
Otherwise, send the current paragraph." t nil)
...
(autoload 'python-x-shell-send-region "python-x" "\
...
Send the section of code at point to the inferior Python process, up to the
current fold or buffer boundaries, then move on to the next." t nil)

(autoload 'python-shell-send-dwim "python-x" "\
Send the current region to the inferior Python process, if active.
Otherwise, use `python-shell-send-current-fold-or-section'" t nil)
...
Restart the current Python process" t nil)
...
Display the inferior Python process in another window." t nil)
...
the send the symbol at point. Print and display the result on the output buffer." t nil)

(autoload 'python-x-setup "python-x" "\
Setup an ESS-like keyboard map in python-mode" nil nil)

(register-definition-prefixes "python-x" '("python-"))
